...
jQuery(document).ready(function ($) {
	...
	$('.payment-body4').off('click', '.currency_list li');
	$('.payment-body4').on('click', '.cms_list li', function () {
		var btn = $(this).closest('.wrp_select_currency').find('.currency_selected');
		btn.find('input').val($(this).data('id'));
		btn.find('span').html($(this).find('b').html());
		btn.find('.balance_icon').remove();
	...
	$('.payment-body').off('click', '.currency_list li');

	// $('.payment-body2').off('click', '.currency_list li');
