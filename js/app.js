...
if (document.querySelector('#header')) {
    ...
    if (document.querySelector('.header__nav')) {
        if ('ontouchstart' in document.documentElement) {
            headerNav.addEventListener("touchstart", openNavMenu);
        } else {
    ...
    //         console.log('after');
    //     } else if (!mediaQuery && window.innerWidth < 1257) {
    //         console.log('prepend');
  ...
  if (document.querySelector('.tabs-sticky')) {
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        tabItems.forEach(function (tab, index) {
            // Используем событие hover для десктопов
            if (!isTouchDevice() && document.body.classList.contains('_pc')) {
